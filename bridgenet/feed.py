"""Rolling activity feed for BridgeNet events.

Stores the last 500 events in TG_BRIDGE_DATA_DIR/bridgenet_feed.json.
...
logger = logging.getLogger("bridge.bridgenet.feed")

_FEED_MAX = 500
_DATA_DIR = os.path.expanduser(os.environ.get("TG_BRIDGE_DATA_DIR", "~/.bridgebot"))
_FEED_FILE = os.path.join(_DATA_DIR, "bridgenet_feed.json")
...
def _load_feed_sync() -> list[dict]:
    ...
    try:
        ...
        logger.error("Failed to load bridgenet feed from %s: %s", _FEED_FILE, e)
        return []
...
def _save_feed_sync(events: list[dict]) -> None:
    ...
    try:
        ...
        logger.error("Failed to save bridgenet feed to %s: %s", _FEED_FILE, e)
...
async def append_event(
    ...
    """
    from .config import BRIDGENET_NODE_NAME
...
async def clear_feed() -> None:
    ...
    async with _lock:
        _save_feed_sync([])
    logger.info("BridgeNet feed cleared")
